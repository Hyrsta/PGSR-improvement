...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, certainty_threshold):
    first_iter = 0
    ...
    img_width, img_height = scene.getTrainCameras()[0].resolution
    roma_model = roma_indoor(device='cuda', upsample_res=(img_height, img_width))
    ...
    ema_multi_view_prior_geo_for_log = 0.0
    ema_multi_view_pho_for_log = 0.0
    normal_loss, geo_loss, prior_geo_loss, ncc_loss = None, None, None, None
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                H_depth, W_depth = render_pkg['plane_depth'].squeeze().shape
                ix, iy = torch.meshgrid(
                    torch.arange(W_depth), torch.arange(H_depth), indexing='xy')
                ...
                if not opt.wo_use_geo_occ_aware:
                    geo_d_mask = (d_mask & (pixel_noise < pixel_noise_th)).clone()
                    weights = (1.0 / torch.exp(pixel_noise)).detach()
                    weights[~geo_d_mask] = 0
                else:
                    geo_d_mask = d_mask.clone()
                    weights = torch.ones_like(pixel_noise)
                    weights[~geo_d_mask] = 0

                if geo_d_mask.sum() > 0:
                    geo_loss = geo_weight * ((weights * pixel_noise)[geo_d_mask]).mean()
                    loss += geo_loss

                if iteration < 15_000:
                    with torch.no_grad():
                        warp, certainty = roma_model.match(viewpoint_cam.image_path, nearest_cam.image_path, device="cuda")

                    certainty_w = certainty.clone().reshape(-1)   # Shape (H*W)
                    warp_im2 = warp[..., 2:].clone().reshape(-1, 2) # Shape (H*W, 2)
                    warp_im2_pixel = to_pixel_coordinates(warp_im2, H_depth, W_depth)
                    # warp_im2_pixel -= 0.5

                    valid = pts_in_nearest_cam[:,2] > 0
                    proj_pixels = torch.zeros((pts_in_nearest_cam.shape[0],2), device=pts_in_nearest_cam.device, dtype=torch.float32)
                    proj_pixels[valid,0] = pts_in_nearest_cam[valid,0] * nearest_cam.Fx / pts_in_nearest_cam[valid,2] + nearest_cam.Cx
                    proj_pixels[valid,1] = pts_in_nearest_cam[valid,1] * nearest_cam.Fy / pts_in_nearest_cam[valid,2] + nearest_cam.Cy

                    prior_pixel_noise = torch.norm(proj_pixels - warp_im2_pixel, dim=-1)  # shape: (H*W)

                    prior_mask = (d_mask) & (certainty_w > certainty_threshold) # Shape (H*W)
                    prior_weights = certainty_w.detach()
                    prior_weights[~prior_mask] = 0

                    if prior_mask.any():
                        prior_geo_loss = lambda_prior(iteration) * 0.03 * ((prior_weights * prior_pixel_noise)[prior_mask]).mean()
                        # prior_geo_loss = 0.03 * ((prior_weights * prior_pixel_noise)[prior_mask]).mean()
                        loss += prior_geo_loss
                    ...
                    if iteration % 200 == 0:
                        with torch.no_grad():
                            ...
                            d_mask_show = (weights.float()*255).detach().cpu().numpy().astype(np.uint8).reshape(H_depth,W_depth)
                            ...
                            certainty_show = (prior_weights.float()*255).detach().cpu().numpy().astype(np.uint8).reshape(H_depth,W_depth)
                            certainty_color_show = cv2.applyColorMap(certainty_show, cv2.COLORMAP_JET)

                            nearest_gt_image, _ = nearest_cam.get_image()   # (3, H, W)
                            im2_transfer_rgb = F.grid_sample(nearest_gt_image[None], warp[..., 2:][None], mode='bilinear', align_corners=False)[0]     # (3, H, W)
                            # white_im = torch.ones_like(im2_transfer_rgb)                           # (3,H,W)
                            # warp_combine = certainty * im2_transfer_rgb + (1 - certainty) * white_im     # (3,H,W)
                            warp_combine = certainty * im2_transfer_rgb
                            warp_combine_show = ((warp_combine).permute(1,2,0).clamp(0,1)[:,:,[2,1,0]]*255).detach().cpu().numpy().astype(np.uint8)   # (H,W,3)

                            noise_n = (prior_pixel_noise - prior_pixel_noise.min()) / (prior_pixel_noise.max() - prior_pixel_noise.min() + 1e-20)
                            noise = (noise_n.clamp(0,1)*255).detach().cpu().numpy().astype(np.uint8).reshape(H_depth, W_depth)
                            noise_color = cv2.applyColorMap(noise, cv2.COLORMAP_JET)      # shape (H,W,3)
                            
                            nearest_gt_image_show = ((nearest_gt_image).permute(1,2,0).clamp(0,1)[:,:,[2,1,0]]*255).detach().cpu().numpy().astype(np.uint8)

                            row0 = np.concatenate([gt_img_show, img_show, normal_show, distance_color, warp_combine_show, noise_color], axis=1)
                            row1 = np.concatenate([d_mask_show_color, depth_color, depth_normal_show, image_weight_color, certainty_color_show, nearest_gt_image_show], axis=1)
                ...
                if geo_d_mask.sum() > 0:
                    if use_virtul_cam is False:
                        with torch.no_grad():
                            ## sample mask
                            geo_d_mask = geo_d_mask.reshape(-1)

                            valid_indices = torch.arange(geo_d_mask.shape[0], device=geo_d_mask.device)[geo_d_mask]
                            if geo_d_mask.sum() > sample_num:
                                index = np.random.choice(geo_d_mask.sum().cpu().numpy(), sample_num, replace = False)
                                valid_indices = valid_indices[index]

                            weights_flat = weights.reshape(-1)
                            pixels_flat = pixels.reshape(-1,2)
                            weights_photo = weights_flat[valid_indices]
                            ## sample ref frame patch
                            pixels = pixels_flat[valid_indices]
                        ...
                        ncc = ncc.reshape(-1) * weights_photo
        ...
        with torch.no_grad():
            ...
            ema_multi_view_prior_geo_for_log = 0.4 * prior_geo_loss.item() if prior_geo_loss is not None else 0.0 + 0.6 * ema_multi_view_prior_geo_for_log
            ...
            if iteration % 10 == 0:
                loss_dict = {
                    ...
                    "prior_geo": f"{ema_multi_view_prior_geo_for_log:.{5}f}",
...
def lambda_prior(iter):
    warm  = 7_000          # ramp-in period
    fade  = 15_000         # start fading after this iter
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, certainty_threshold)
